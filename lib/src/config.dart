...
/// Configuration passed to `LogKit.init`.
///
/// Only the first successful `LogKit.init` call's config is used for the
/// lifetime of the isolate — see `LogKit.init` for the idempotency contract.
class LogKitConfig {
  const LogKitConfig({
  ...
  /// When true (default), registers a `log_kit` `MethodChannel` handler so
  /// native (Kotlin/Swift) app code can log through `LogKitNative` — see
