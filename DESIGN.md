...
あったアプリ固有判断はすべて呼び出し側の `LogVaultConfig` に追い出す。

## 3. パッケージ概要

- **パッケージ名**: `log_vault`（`flutter_plugin/log` ディレクトリはそのまま、
  `pubspec.yaml` の `name: log_vault` とする。当面はローカル path 依存での
  利用を想定）
...
> **`log_vault` 単一パッケージに `share_plus` を含める**方針とする
...
log_vault/
  lib/
    ...
    src/
      ...
      log_vault.dart                # LogVault facade (singleton的 static API)
      config.dart                 # LogVaultConfig
      sinks/
...
- `LogVault.init` の idempotent 定義（レビュー反映で明確化）:
...
```dart
LogVault.d('画面遷移: Home -> Chat', tag: 'Nav');
LogVault.e('token refresh failed', error: e, stackTrace: st, tag: 'Auth');
```
...
2. `print.dart` の `customLogger`/`logger` 呼び出し箇所（100+ 箇所）は、
   まず `print.dart` 内部で `LogVault.d/i/w/e/v` に委譲する薄いラッパーとして
   残し、一括置換はしない（既存コールサイトを壊さないため）
3. `log_share.dart: shareAppLogs()` を `LogVault.shareLogs()` 呼び出しに置き換え
4. dev-build 判定・Crashlytics連携は `LogVaultConfig` / `addSink` に移設
5. 動作確認後、余力があれば `print.dart` の薄ラッパーを段階的に `LogVault` 直呼びへ
   置換（本設計のスコープ外、別タスク）
...
`LogVaultConfig`, `FileSink`, `LogDumper`）から実装に着手する。
...
- **ダンプ zip の一時ファイル漏れを解消**: `LogDumper` を毎回使い捨てで
  ...
  以下の zip がシステム一時ディレクトリに残り続けていた。`LogVault` が
  `LogDumper` を単一インスタンスとして保持し、次回 `dumpLogs()` 呼び出し時に
  前回分を自動削除する方式に変更。加えて `LogVault.disposeLastDump()` /
...
- **`LogVault.init()` 前のログ呼び出しが release ビルドで完全に消える問題を修正**:
...
いたため、`log_vault` を（Dart単体パッケージではなく）**真の Flutter plugin**
として実装した。

### 方式

ネイティブ→Dart の `MethodChannel` 呼び出しで `LogVault` に転送する方式を採用
（採用理由: ローテーション/保持/フォーマット/リダクションのロジックを
Dart側に一本化でき、Kotlin/Swiftに再実装・再保守する必要がない）。

- **制約**: `FlutterEngine` が起動し、Dart側isolateでチャンネルハンドラが
  登録された後（= `LogVault.init()` 完了後）でなければネイティブ→Dartの呼び出しは
...
- `pubspec.yaml` に `flutter.plugin.platforms.android/ios` を追加し、
  plugin化。
- **Android**: `android/src/main/kotlin/.../LogVaultPlugin.kt`
  （`FlutterPlugin` 実装、`onAttachedToEngine`/`onDetachedFromEngine`で
  `MethodChannel` の attach/detach）+ `LogVaultNative.kt`（アプリの任意の
  Kotlin/Javaコードから呼べる `LogVaultNative.d(tag, message)` 等の
  静的API、内部でメインスレッドへ`invokeMethod`）。
- **iOS**: `ios/Classes/LogVaultPlugin.swift`（`FlutterPlugin`実装、
  `register(with:)`でチャンネル登録）+ `LogVaultNative.swift`（同様の
  `LogVaultNative.d(tag:message:)` 等）。
- **Dart**: `lib/src/native_bridge.dart` の `NativeLogBridge` が
  `MethodChannel('log_vault')` の `'log'` メソッド呼び出しを受け取り、
  `LogEntry` を再構築して `LogVault` の全 sink（`FileSink` 含む）へ配布する。
  `LogVault.init()` 内で `LogVaultConfig.enableNativeBridge`（デフォルト`true`）
  に応じて attach。
- ネイティブ側は `timestampMillis`（送信時刻）を payload に含め、Dart側の
  受信処理時刻とのズレ（チャンネル往復レイテンシ）を排除している。
- `LogVault._dispatch()` を新設し、Dart呼び出し経由（`LogVault.d/i/w/e/v`）と
...
- **`LogVault.init()` がバインディング未初期化のIsolateでクラッシュする問題を
  ...
  この例外が `LogVault.init()` を reject し、かつ `_initStarted` が例外前に
...
- **`CallbackLogSink` 経由でRedactionが素通りする問題を修正**: Redactionは
  ...
  外部送信し得た。`LogVault.formatter`（設定済みフォーマッタを返すgetter）を
...
- **Android: マルチエンジン構成で先に破棄されたエンジンが後発エンジンの
  ログを止めてしまう問題を修正**: `LogVaultPlugin.onDetachedFromEngine` が
  無条件に `LogVaultNative.detach()`（`channel = null`）していたため、
  `FlutterEngineGroup`/add-to-app で複数エンジンが動く構成では、後から
  attachしたエンジンが生きているのに先に破棄されたエンジンのdetachで
  ネイティブログが全て黙って消える事故があり得た。`LogVaultPlugin` が自分の
  channelインスタンスを保持し、`LogVaultNative.detachIf(expected)` で
...
- **`LogVault.init()` の2回目呼び出しが1回目の完了を待たずに解決する問題を
...
回帰テストを `test/log_vault_test.dart` に追加（init並行性テスト、sink例外
分離テスト）。テスト総数34件、全通過。
